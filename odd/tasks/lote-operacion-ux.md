...
- [x] S1 Servicios a Catálogos (HECHO 13e0304 2026-09-23; verificado 2026-09-25). Servicios salió del panel de admin a su propia ruta `/services` (`app/app/services/page.tsx` + `services-client.tsx`); los tabs del admin ya no lo incluyen (`admin-tabs.tsx:21-30`) y `admin-sections/` no tiene sección de servicios. La agrupación "Catálogos" existe en la navegación (`main-nav.tsx:45-60`: Inventario + Servicios). Decisión del usuario 2026-09-25: productos y servicios quedan como están, en rutas separadas agrupadas en el menú; NO se crea un `/catalogos` unificado. Residuo cosmético detectado: la descripción del link a `/admin` en `main-nav.tsx` todavía dice "Empleados, servicios y precios".
...
- [x] N3 Comisión producto/servicio/personalizado: modelo definido e IMPLEMENTADO (HECHO d509da8 + c9599b9 2026-09-24; re-verificado 2026-09-25). La nota "implementación PENDIENTE BLOQUEADA POR DATOS / NO implementado" era del 2026-09-23 y quedó stale: el código de HEAD ya NO aplica el % del empleado a todo lo que no sea `custom`. Modelo implementado: servicio → % del empleado sobre el subtotal, origen `percent`, siempre nómina, nunca inmediato (`commissions/schemas.ts:183,242`; `commissions/service.ts:270`); producto → valor fijo `commission_value` × cantidad (o regla ítem × empleado), NUNCA el % del empleado, pagable de inmediato si `payout_mode="inmediato"` (`commissions/schemas.ts:206,256`; `invoices-client.tsx:942`); personalizado → el operador elige comisión (valor) / porcentaje / ninguna, mutuamente excluyentes (`invoices-client.tsx:79,147`; `billing/schemas.ts:138-182`). Migración 030 (`commission_mode`, `commission_percent_override`) presente y consumida en nómina (`payroll/service.ts:546,607,735`), pago inmediato (`commissions/service.ts:191,254,270-279`) y detalle (`billing/service.ts:178,430`). Tests: `tests/commission-mode.test.ts` (26 casos; suite 349/349). Modelo original del usuario: servicio = % del empleado (por servicios trabajados); producto = valor fijo de comisión (valor neto por vender); personalizado = elegible entre % / comisión / ninguna (porque es libre). El código HOY aplica el % del empleado a TODO lo que no sea `custom` (incluido producto), así que el cambio es de CÁLCULO DE NÓMINA, no cosmético. Datos medidos en PRUEBAS: 3 productos, 0 con comisión cargada; 6 empleados por porcentaje; 0 períodos cerrados. Conclusión: NO se implementa todavía — activar el cálculo antes de cargar los valores haría que los productos pasen a pagar $0, bajándole el sueldo a quien hoy gana por porcentaje. Secuencia correcta: (1) cargar el valor de comisión de los productos, (2) cambiar el cálculo, (3) verificar con nómina de prueba. Pregunta abierta para el usuario: cuánto vale la comisión de cada uno de los 3 productos; no se puede inventar). CIERRE 2026-09-25: la implementación está en HEAD; lo que sigue pendiente es solo acción de datos/deploy — aplicar la 030 en PRUEBAS y cargar el `commission_value` de los 3 productos.
